...
xray_conf_add() {
    ...
    if [[ ${tls_mode} == "TLS" ]]; then
        wget --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_Nginx_Fancyindex/main/VLESS_tls/config.json -O config.json
        modify_path
        modify_inbound_port
    elif [[ ${tls_mode} == "XTLS" ]]; then
        wget --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_Nginx_Fancyindex/main/VLESS_xtls/config.json -O config.json
        xray_xtls_add_more
    elif [[ ${tls_mode} == "None" ]]; then
        wget --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_Nginx_Fancyindex/main/VLESS_tls/config.json -O config.json
        modify_listen_address
...
acme_cron_update() {
    ...
    *)
    ...
    [yY][eE][sS] | [yY])
        if [[ "${ssl_self}" != "on" ]]; then
            wget -N -P ${guyezi_dir} --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_Nginx_Fancyindex/main/ssl_update.sh && chmod +x ${ssl_update_file}
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/guyezi/Xray_Nginx_Fancyindex/main/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ ${shell_version} != ${newest_version} ]]; then
        ...
        [yY][eE][sS] | [yY])
            ...
            wget -N --no-check-certificate -P ${guyezi_dir} https://raw.githubusercontent.com/guyezi/Xray_Nginx_Fancyindex/main/install.sh && chmod +x ${guyezi_dir}/install.sh
